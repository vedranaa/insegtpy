...
struct im_st
{
    ...
    int rows, cols, layers, n_pix, n_im; // rows, cols and layers are the image dimensions and n_pix = rows*cols
};
...
void build_km_tree_multi ( im_st& im, tree_st& tree, int n_train, bool normalize ) {
    ...
    for (int j = 0; j < im.n_im; j++ )
    {
        ...
        for (int i = 0; i < n_train; i++ )
        {
            ...
            c = r_id[i]/rows_c; // column is floored because of int
            r = r_id[i]-c*rows_c; // row is rest after column
            patches[pid].idx = 0; // inital id is 0
            patches[pid].patch_data = im_patch_data + idx; // pointer to patch memory
            sample_patch_multi(im, tree, j, patches[pid], r + tree.Mh, c + tree.Mh, normalize); // sampel in image with added boundary
            idx += tree.n_dim; // step number of patch pixels forward
        }
    }
    // k-means tree
                                    ...
                                    bool normalize, double *tree) 
{
    ...
    // build the km-tree using C++ subroutine
    build_km_tree_multi ( Im, Tree, n_train, normalize );
